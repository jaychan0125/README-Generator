...
function renderLicenseSection(license, licenseLink) {
  return license === 'none' ? '' : `## License <br/>${licenseLink}`
}
...
function contributorLinks(contributorData) {
  ...
  return contributors.length < 1 ? '' : '## Contributors and Questions <br/>If you have questions, please visit our profiles: <br/>' + contributors.map(contributor => `[${contributor}](https://github.com/${contributor})`).join('<br/>');
}
